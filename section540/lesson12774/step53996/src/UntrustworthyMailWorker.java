 ...
 * работника почты, который вместо того, чтобы передать
 * почтовый объект непосредственно в сервис почты,
 * последовательно передает этот объект набору третьих лиц,
 * а затем, в конце концов, передает получившийся объект
 * непосредственно экземпляру RealMailService. У
 * UntrustworthyMailWorker должен быть конструктор от
 * массива MailService ( результат вызова processMail
...
import stepikClasses.RealMailService;
...
public class UntrustworthyMailWorker implements MailService {
    ...
    public UntrustworthyMailWorker(MailService[] agents) {
        this.agents = agents;
    }
